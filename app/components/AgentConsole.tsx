...
export function AgentConsole() {
  const { sessions, switchSession, appendMessage, updateAgentCase } = useSessions();
  ...
  useEffect(() => {
    if (selectedSessionId && !requestSessions.some((session) => session.id === selectedSessionId)) {
      setSelectedSessionId(null);
    }
  }, [requestSessions, selectedSessionId]);

  useEffect(() => {
    setReplyDraft("");
    setNotesDraft(selectedSession?.agentCase?.resolutionNotes ?? "");
  }, [selectedSession?.id, selectedSession?.agentCase?.resolutionNotes]);
  ...
  return (
    <div className="flex h-full min-h-0 overflow-hidden bg-ink text-text">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        {selectedSession ? (
          <div className="flex min-h-0 flex-1 flex-col 2xl:flex-row">
            <section className="min-h-0 flex-1 overflow-y-auto px-3 py-4 sm:px-4 md:px-6 md:py-6 xl:px-8">
              <div className="mx-auto max-w-4xl space-y-5">
                {selectedSession.messages.map((message) => <MessageBubble key={message.id} message={message} />)}
              </div>
            </section>
            <aside className="flex min-h-0 w-full flex-shrink-0 flex-col border-t border-border bg-surface/70 p-4 sm:p-5 2xl:w-[360px] 2xl:border-l 2xl:border-t-0">
              <div className="flex min-h-0 flex-col space-y-5 2xl:flex-1 2xl:overflow-y-auto 2xl:pr-1">
